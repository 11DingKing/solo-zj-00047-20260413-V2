...
const Layout: FC = (): ReactElement => {
    ...
    const onCreateSubTask = (itemId: string, subTask: { name: string; completed?: boolean }) => {
        if (appContext.state.selectedList) {
            actions.items.createSubTask(appContext.state.selectedList.id, itemId, subTask);
        }
    }

    const onUpdateSubTask = (itemId: string, subTaskId: string, subTask: { name?: string; completed?: boolean }) => {
        if (appContext.state.selectedList) {
            actions.items.updateSubTask(appContext.state.selectedList.id, itemId, subTaskId, subTask);
        }
    }

    const onDeleteSubTask = (itemId: string, subTaskId: string) => {
        if (appContext.state.selectedList) {
            actions.items.deleteSubTask(appContext.state.selectedList.id, itemId, subTaskId);
        }
    }
