...
export interface ItemActions {
    ...
    createSubTask(listId: string, itemId: string, subTask: { name: string; completed?: boolean }): Promise<TodoItem>
    updateSubTask(listId: string, itemId: string, subTaskId: string, subTask: { name?: string; completed?: boolean }): Promise<TodoItem>
    deleteSubTask(listId: string, itemId: string, subTaskId: string): Promise<TodoItem>
}
...
export const createSubTask = (listId: string, itemId: string, subTask: { name: string; completed?: boolean }): ActionMethod<TodoItem> => async (dispatch: Dispatch<SaveItemAction>) => {
    const itemService = new ItemService(config.api.baseUrl, `/lists/${listId}/items`);
    const newItem = await itemService.createSubTask(itemId, subTask);
...
export const updateSubTask = (listId: string, itemId: string, subTaskId: string, subTask: { name?: string; completed?: boolean }): ActionMethod<TodoItem> => async (dispatch: Dispatch<SaveItemAction>) => {
    const itemService = new ItemService(config.api.baseUrl, `/lists/${listId}/items`);
    const newItem = await itemService.updateSubTask(itemId, subTaskId, subTask);
...
export const deleteSubTask = (listId: string, itemId: string, subTaskId: string): ActionMethod<TodoItem> => async (dispatch: Dispatch<SaveItemAction>) => {
    const itemService = new ItemService(config.api.baseUrl, `/lists/${listId}/items`);
    const newItem = await itemService.deleteSubTask(itemId, subTaskId);
